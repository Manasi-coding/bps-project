...
class GraspEstimator(Node):
    def __init__(self):
        ...
        self.latest_cloud = None  # kept as a compatibility flag for log_results.py's warm-up wait loop
    ...
    def estimate(self) -> Optional[EstimationResult]:
        ...
        valid = np.isfinite(zs) & (zs > 0.05) & (zs < 12.0)
    ...
    def _estimate_dimensions(self, vs, us, zs):
        """Back-project masked pixels (row indices vs, col indices us, depth
        zs) to 3D via the pinhole camera model, then estimate the object's
        bounding extent via PCA + percentile spread. Matches how estimate()
        actually calls this (pixel-space mask, not a pre-built point array)."""
